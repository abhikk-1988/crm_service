    ...
    ( function ( Pace, $ , bmh_pusher) {

        // module dependency of ngRoute module 
        app = angular.module ( 'app', ['ngRoute', 'ngSanitize', 'ngAnimate', 'ui.bootstrap','ui.router','uiSwitch','isteven-multi-select','ui.timepicker','ui.toggle','ckeditor','chart.js'] );
        ...
        app.value ( 'appUrls', {
            baseUrl: 'http://crm.bookmyhouse.biz/crm/',
            stuffUrl: 'http://crm.bookmyhouse.biz/crm/stuff/',
            appUrl: 'http://crm.bookmyhouse.biz/crm/app/',
            apiUrl: 'http://crm.bookmyhouse.biz/crm/apis/'
        });
        ...
        app.value('hide_cal_for_sub_status', [19,20,21,39,40,41,42,43,46,52]);
          
        // Activity Ids for which remarks are mandatory
        app.value('mandatoryRemarksActivity',[3,6,47,11,44,22,14,45,23,52,34,5,7,16,49,17,48]);
        
        app.constant ('baseUrl', 'http://crm.bookmyhouse.biz/crm/');
        ...
        var email_template_events = [
            ...
            {title: 'Re-Assign To SP', value : 're_assign_sp'},
            {title : 'Multiple Project Lead Assignment ASM', value : 'multiple_project_lead_assignment_level_2'}
        ];
        ...
        app.config ( function ( $routeProvider, baseUrl, $locationProvider, $stateProvider, $urlRouterProvider) {
            ...
            $routeProvider.
                    ...
                    when ('/home', {
                        ...
                        templateUrl: baseUrl + 'html/home.php',
                        controller: 'homeCtrl',
                        resolve: {
                            login_user: function ( $http, $location, $q ) {
                    ...
                    when ('/re-assign-lead/:user_id?', { // Umesh Chandra Katiyar
                    ...
                    when ('/re-assign-to-sp/:user_id?', { // Umesh Chandra Katiyar
                        templateUrl: baseUrl + 'templates/reAssignLeadsToSp.html',
                        controller: 'reAssignLeadToSpCtrl',
                        resolve: {
                            user_auth: function ( $http, $location ) {
                                return $http ( {
                                    url: baseUrl + 'apis/getCurrentUser.php',
                    ...
                    when ('/lead-date-extend/:user_id?', { // Umesh Chandra Katiyar
                        templateUrl: baseUrl + 'templates/leadExtendDateList.html',
                        controller: 'leadExtendDateListCtrl',
                        resolve: {
                            user_auth: function ( $http, $location ) {
                                return $http ( {
                                    url: baseUrl + 'apis/getCurrentUser.php',
                    ...
                    when ( '/assigned-enquiries', {  
                        templateUrl: baseUrl + 'templates/assigned_enqueries.html',
                        controller: 'assignedEnqueriesCtrl',
                        resolve: {
                            user_session : function (httpService){
                    ...
                    when ('/project-mailer',{
                        templateUrl : baseUrl + 'templates/project_mailer.html',
                        controller: 'projectMailerCtrl',
                        resolve: {
                            user_session : function (httpService){
                                return	httpService.makeRequest({
                                            url : baseUrl + 'apis/getCurrentUser.php',
                    ...
                    when ('/addNewMailer', {
                        templateUrl : baseUrl + 'templates/addMailer.html',
                        controller : 'addMailerCtrl',
                        resolve: {
                            user_session : function (httpService){
                                return	httpService.makeRequest({
                                            url : baseUrl + 'apis/getCurrentUser.php',
                            ...
                            alreadyExistsMailerProjects : function (httpService){
                                return httpService.makeRequest({
                                    url     : baseUrl+'apis/getAlreadyExistsMailerProjects.php',
                                    method  : 'GET'
                                }).then(function (resp){
                                    return resp.data;
                                });
					...
					when ('/crm-teams',{
                        templateUrl : baseUrl +'templates/crm_teams.html',
                        controller: 'crmTeamsCtrl',
                        resolve: {
                        }
                    }).
                    otherwise ( {
            ...
            $rootScope.$on ( '$routeChangeStart', function ( event, next, current ) {
                ...
                authPromise.then ( function ( success ) {
                    ...
                    if ( parseInt ( success.data ) === 0 ) {
						...
						// Get instance of pusher client and disconnect from it 
                        bmh_pusher = new pusherClient();
                        bmh_pusher.disconnectPusher();
                    ...
                    /**
					 * if server is unavailable to respond or any server side issue is there 
					 * then we have to prevent user to login in system 
					 * Also disconnect from pusher forcely
					 */
					...
					// destroying user session from service
...
//                            $scope.changeAppLayout ( '1119px', '230px', true );
            ...
            $scope.stopPusher = function (user_id){
                ...
                if(bmh_pusher){
                   bmh_pusher.disconnectPusher();
                    ...
                    bmh_pusher.unbindEvents();
                    bmh_pusher.unsubscribeFromChannels(channel_prefix+'@'+user_id);
                    bmh_pusher.disconnectPusher();
                }
            };
            ...
            $scope.findDifferences_not_working = function (objectA, objectB) {
                ...
                /**
                * Function to get difference between object
                */
                $scope.findObjDiff = function (obj1, obj2){

                    var diff = new Array;

                    // obj1 will be the main object 
                    // obj2 will be the cloned object
                ...
                $scope.createHistory = function (history_obj){
                    $http.post(baseUrl+'apis/createHistory.php',history_obj);
                }
         ...
         * Mapping Controller 
         */
        app.controller ( 'mapping', function ( $scope, utilityService, $routeParams, assigned_status, $location, $route, user_auth , $filter) {
            ...
            $scope.selectStatus = function ( event, status ) {
                ...
                if ( angular.element ( event.currentTarget ).prop ( 'checked' ) === true ) {
                    ...
                    var status_obj = {
                        ...
                        'status_active_state':status.is_activity_status
                    };
                    
                    $scope.assigned_status.push (status_obj );
                }
                else {
                    
                    var index = $scope.assigned_status.indexOf ( status );
                    
                    var removed_status = $filter('filter')($scope.assigned_status,status.status_id,true);
                    
                    $scope.assigned_status.splice ( removed_status, 1 );
                
                    var modified_assigned_status = $scope.assigned_status.filter(function (element){
                        
                        if(element.status_id != status.status_id){
                            return element;
                        }
                    });
                    
                    $scope.assigned_status = modified_assigned_status;
                }
            };
            ...
            $scope.checkitem = function ( value ) {
                ...
                for ( var i = 0; i < $scope.assigned_status.length; i ++ ) {
                    ...
                    if ( parseInt ( value ) === parseInt($scope.assigned_status[i].status_id) && $scope.assigned_status[i].status_active_state == 1) {
                        checked = true;
                    }
                }
        ...
        /**
         * Directive To format date string
         */
        ...
        app.directive('highlightTr' , function (){
            ...
            return {
                ...
                link : function (scope, element, attr)
                {
                    ...
                    var event_timestamp = new Date(scope.date).getTime();

                    if(new Date(event_timestamp).getDate() < new Date().getDate()){
          ...
          // sms controller
        app.controller('smsCtrl', function ($scope, baseUrl,$http){
            
            // Enquiry id from parent controller
            var self = $scope;

            self.sms_data  = {};

            // getClient number from enquiry id
            $http.post(baseUrl+'apis/helper.php?method=getLeadCustomerInfo&params=enquiry_id:'+self.enquiry_id).then(
                function (resp){
                    
                    if(resp.data){
                        self.sms_data.number = resp.data.customerMobile;
                    }
                }
            );

            self.sendSms = function (data){
                ...
                // Call sms apis to send sms
                $http.get(baseUrl + 'apis/sendsms.php?text='+data.message+'&number='+data.number).then(
                    function (resp){
                        
                        if(resp.data){

                            // check in response LogID
                            var logid = resp.data.split('=');
                            ...
                            if(logid[0] === 'LogID'){
                                ...
                                $scope.createHistory({
                                    ...
                                    lead_number : self.lead_id,
                                    type: 'new',
                                    meta_data : {
                                        display_type:'popup',
                                        data: data.message,
                                        added_by : self.currentUser.firstname+' '+self.currentUser.lastname
                                    },
                                    employee_id: self.currentUser.id,
                                    details : 'An sms is sent to client on number ('+data.number+')  by '+ self.currentUser.firstname+' '+self.currentUser.lastname
                                });
        ...
        app.controller('emailCtrl', function ($scope,baseUrl,$http,projectMailerService,$filter){
            ...
            // $scope.email_data.toEmail = [];

            $scope.mailerProjects = [];

            projectMailerService.getAllProjectMailers().then(
                function (resp){

                    if(resp.data){
                        $scope.mailerProjects = resp.data;
                        // console.log($scope.mailerProjects);
                    }
                }
            );

            // getClient number from enquiry id
            $http.post(baseUrl+'apis/helper.php?method=getLeadCustomerInfo&params=enquiry_id:'+$scope.enquiry_id).then(
                function (resp){
                    
                    if(resp.data){
                        $scope.email_data.toEmail = resp.data.customerEmail;
                    }
                }
            );

            // function to get mailer template and attachments
            $scope.getProjectTemplate = function (mailer){

                if(!mailer){
                    $scope.email_data.content = null;
                    return;
                }

                $scope.email_data.content = mailer.content;
                $scope.email_data.attachments = mailer.attachments;
            };

            // To remove attachments
             $scope.removeAttachment = function (attachment_name){

                var attachment_to_remove = $filter('filter')($scope.email_data.attachments,{save_name:attachment_name},true);
                var index = $scope.email_data.attachments.lastIndexOf(attachment_to_remove[0]);
                // remove by index
                $scope.email_data.attachments.splice(index,1);
            }
            ...
            $scope.sendMail = function (email){
                ...
                // All ok post form
                $http({
                    url : baseUrl + 'apis/sendEmail.php',
                    method: 'POST',
                    data : $.param({
                    ...
                    function (resp){
                        ...
                        if( parseInt(resp.data.success) === 1){
                            $scope.email_data = {};
                            $scope.toggleEmailBox();
                            // create history    
                            $scope.createHistory({
                                enquiry_id: $scope.enquiry_id,
                                lead_number : $scope.lead_id,
                                type: 'new',
                                employee_id: $scope.currentUser.id,
                                details : 'An email is sent to client on email address ('+ email.toEmail+')  by '+ $scope.currentUser.firstname+' '+$scope.currentUser.lastname
                            });
                        ...
                        // Errors 
                           ...
                           if(Object.keys(resp.data.errors.invalid_emails).length > 0){

                                error_message += '<ol>';
                                angular.forEach(resp.data.errors.invalid_emails, function (val,key){
                                    error_message += '<li>' + val.email + '</li>';
                                });
                                error_message += '</ol>';
                           }
                           
                           showToast(error_message,'Invalid emails','error');
                           return false;
                        }else{
                            error_message = resp.data.message;
                            showToast(error_message,'Error','error');
                            return false;
                        }
