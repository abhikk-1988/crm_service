...
if((!empty($_GET))){	// Check whether the date is empty	
	...
	$filename = 'sales_wise_meeting_report_from_'.$startDate."_to_".$endDate;
	
	// Monthly/Yearly Report of Signle Project
	if(isset($startDate) && isset($endDate)){
		
		$headers = array('TL','SP','Deal Closed','Follow Up', 'Meeting Done','Meeting Not Done','Blank','Grand Total');
		
		$project = array('Deal Closed','Follow Up', 'Meeting Done','Meeting Not Done','Blank');
...
function getCount($projectName, $spId, $startDate, $endDate){
	...
	if($projectName == 'Deal Closed'){
		
		$SQL_Lead = "SELECT count(DISTINCT Q.enquiry_id) as total FROM lead_status AS Q WHERE Q.user_id = '$spId' AND Q.disposition_status_id = 7 AND Q.date > '$startDate' AND Q.date < '$endDate'";
		
		
	}elseif($projectName == 'Follow Up'){
		
		$SQL_Lead = "SELECT count(DISTINCT Q.enquiry_id) as total FROM lead_status AS Q WHERE Q.user_type = 'sales_person' AND Q.user_id = '$spId' AND Q.disposition_status_id IN (3,4) AND Q.disposition_sub_status_id IN (12,22, 10,18,37) AND Q.date > '$startDate' AND Q.date < '$endDate'";

	}elseif($projectName == 'Meeting Done'){
		
		$SQL_Lead = "SELECT count(DISTINCT Q.enquiry_id) as total FROM lead_status AS Q WHERE Q.user_type = 'sales_person' AND Q.user_id = '$spId' AND Q.disposition_status_id = 3 AND Q.disposition_sub_status_id = 11 AND Q.date > '$startDate' AND Q.date < '$endDate'";

	}elseif($projectName == 'Meeting Not Done'){
		
		$SQL_Lead = "SELECT count(DISTINCT Q.enquiry_id) as total FROM lead_status AS Q WHERE Q.user_type = 'sales_person' AND Q.user_id = '$spId' AND Q.disposition_status_id = 3 AND Q.disposition_sub_status_id = 44 AND Q.date > '$startDate' AND Q.date < '$endDate'";
		
	}elseif($projectName == 'Blank'){
		
		$sqlLead = "SELECT count(DISTINCT Q.enquiry_id) as total FROM lead AS Q WHERE Q.enquiry_id NOT IN (SELECT enquiry_id FROM lead_status AS P WHERE P.user_type='sales_person') AND Q.lead_assigned_to_sp = '$spId' AND Q.lead_assigned_to_sp_on > '$startDate' AND Q.lead_assigned_to_sp_on < '$endDate'";
