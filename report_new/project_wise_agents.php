...
if((!empty($_GET))){	// Check whether the date is empty	
	...
	$filename = 'CRM_Meeting_Generated_From_'.$startDate.'_to_'.$endDate;
	...
	if(isset($startDate) && isset($endDate)){
		...
		$sqlQuery = "SELECT A.id FROM employees AS A, employees AS B WHERE A.reportingTo != 0 AND A.reportingTo = B.id AND A.isDelete=0 AND B.isDelete=0 AND A.designation=9 ORDER BY B.firstname";
...
function getCount($projectName, $spId, $startDate, $endDate){
	...
	$SQL_Lead = "SELECT count(DISTINCT Q.enquiry_id) as total FROM lead_status AS Q
				
	JOIN lead_enquiry_projects AS P ON P.enquiry_id = Q.enquiry_id
				
	WHERE Q.user_id = '$spId' AND user_type='agent' AND P.project_name = TRIM('$projectName') AND Q.disposition_status_id = 3 AND Q.date > '$startDate' AND Q.date < '$endDate'";
