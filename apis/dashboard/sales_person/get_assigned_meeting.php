...
if(isset($_GET['filter_date_from']) && $_GET['filter_date_from'] != ''){
    ...
    $where_meeting_date = ' AND DATE(leadUpdateDate) = "'.$date_filter_1.'"';
}
...
if($date_filter_1 != '' && $date_filter_2 != ''){
    ...
    $where_meeting_date = ' AND DATE(leadUpdateDate) BETWEEN "'.$date_filter_1.'" AND "'.$date_filter_2.'"';
}
...
// Total Assigned Lead to sales person
$total_assigned_count = $total_records;

// Total accepted count
$total_accepted_meeting     = 0;
$total_accepted_meeting_enquiry_numbers     = array();

// Status wise total count
...
if($result && $total_records > 0){
    ...
    while($row = mysql_fetch_assoc($result)){
        ...
        if(is_null($row['reassign_user_id']) && $row['is_lead_accepted'] == 1 && $row['disposition_status_id'] == 3){
            array_push($total_accepted_meeting_enquiry_numbers, $row['enquiry_id']);
            $total_accepted_meeting++;
        }else{
            
            if($row['rejected_enquiry'] != ''){
                
                // Get first assigned status of this removed enquiry
                ...
                if(mysql_num_rows($get_first_status) > 0){
                    ...
                    if($first_status_object -> disposition_status_id == 3){
...
$fetch_meetings = 'SELECT enquiry_id, count(enquiry_id) as total , leadStatus FROM lead WHERE (lead_assigned_to_sp = '.$user_id.' OR reassign_user_id = '.$user_id.') AND is_lead_accepted = 1 
 AND disposition_status_id  = 3 AND disposition_sub_status_id IN (11,22) AND lead_closure_date IS NULL '.$where_meeting_date.'
 group by leadStatus;'; 
...
echo json_encode(array(
    'total_accepted_meeting' => (int) $total_accepted_meeting, 
