...
if(isset($data['hot_projects'])){
    $hot_projects = $data['hot_projects'];
}
...
//    }
    ...
    if($data['lead_source']['primary']['source_id'] != ''){
        $lead['leadPrimarySource'] = $data['lead_source']['primary']['source_id'];
    }else{
        $errors['lead_primary_source'] = 'Please select lead primary source';
    }
    
    if($data['lead_source']['secondary']['source_name'] != ''){
        $lead['leadSecondarySource'] = $data['lead_source']['secondary']['source_name'];
    }else{
        $errors['lead_secondary_source'] = 'Please select lead secondary source';
    }
...
//    }
    ...
    $errors['lead_source']= 'Please select lead source';
}
    ...
    foreach($enquiry as $key => $val){
        ...
        if($key == 'status_remark'){
             ...
             if($val != ''){
                ...
                if($data['remarks_mandatory']){
                    $errors['enquiry_remark'] = 'Please enter remark';    
                }else{
                    $lead['enquiry_status_remark'] = '';
                }
             }
         }
...
#Start Add lead and LMS sync data by sudhanshu#
//$sql_fetch = "SELECT id FROM crm_enquiry_capture WHERE phone = '".$lead['customerMobile']."' OR email = '".$lead['customerEmail']."'";

// Query modified by abhishek agrawal on 26th Oct 2017 
$sql_fetch = "SELECT id FROM crm_enquiry_capture WHERE phone = '".$lead['customerMobile']."'";

$sql_fetch_result = mysql_query($sql_fetch);

if(mysql_num_rows( $sql_fetch_result) > 0)
{
	$update_query = "UPDATE crm_enquiry_capture "
                . " SET enquiry_assign_by_to_agent_id = '".$lead['lead_added_by_user']."', agent_assign_status = '1', agent_assign_date = '".date('Y-m-d')."',agent_assign_time = '".strtotime(date('Y-m-d H:i:s'))."' "
                . " where phone = '".$lead['customerMobile']."'";
	mysql_query($update_query);
}else{
	$insert_query = "INSERT INTO crm_enquiry_capture (query_request_id,created_time,created_on,enquiry_from,syn_in_crm,phone,email,name,ivr_push_type,ivr_push_status,enquiry_assign_to_agent_id,agent_assign_status,agent_assign_date,agent_assign_time) VALUES('PORT".rand() ."','".strtotime(date('Y-m-d H:i:s'))."','".date('Y-m-d')."','".$lead['leadSecondarySource']."','1','".$lead['customerMobile']."','".$lead['customerEmail']."','".$lead['customerName']."','Pending','Pending','".$lead['lead_added_by_user']."','1','".date('Y-m-d')."','".strtotime(date('Y-m-d H:i:s'))."')";
	mysql_query($insert_query);	
}

#END Add lead and LMS sync data by sudhanshu#
// Create New Lead
...
$new_date = $callback_date." ".$timevalue.":00"; 
    if($sub_status_id == ''){
        $sub_status_id = 0;
		 ...
		 $urls = "";
		 $stickyarray = array('transaction_id'=>'CTI_UPDATE_STICKY','agent_id'=>$user['crm_id'],'camp_name'=>'Bookmyhouse','mobile' => $lead['customerMobile']);
		 foreach($stickyarray as $keys => $values){
				$urls .= urlencode($keys).'='.urlencode($values).'&';
			}
		$hitsticky = "https://admin.c-zentrixcloud.com/apps/appsHandler.php";
    ...
    mysql_query('INSERT INTO lead_status (lead_id, enquiry_id, disposition_status_id, disposition_sub_status_id,hot_warm_cold_status, user_type, user_id, remark) VALUES ("'.$lead_id.'",'.$enquiry_id.','.$status_id.','.$sub_status_id.',"'.(isset($lead['leadStatus']) && $lead['leadStatus']!= '' ? $lead['leadStatus'] : NULL).'","agent",'.$user['id'].',"'.$remark.'")');
...
//			sendLeadAssginementMailToTLCRM($internal_mail_data);
        ...
        case '1': // Not Interested 0
            ...
            // Send Reminder mail
			sendSimpleReminderMail($enquiry_id,$hot_projects);
            break;      
...
//    createLog($leadAssignLogTLCRM);
    ...
    if($agent_asm_id != '' && ($status_id == 3 || $status_id == 6)){
