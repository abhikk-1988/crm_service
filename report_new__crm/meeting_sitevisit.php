    ...
    if((!empty($_GET))) {	// Check whether the date is empty	
        ...
        if(isset($startDate) && isset($endDate)){
        	...
        	$filename = 'all_meeting_sitevisit_from_'.$startDate."_to_".$endDate;
           	...
           	LEFT JOIN employees as d ON (d.id = a.lead_assigned_to_asm)
           	
           	LEFT JOIN employees as e ON (e.id = a.lead_assigned_to_sp)
           	...
           	LEFT JOIN disposition_status_substatus_master as i ON i.id = (SELECT disposition_status_id FROM lead_status WHERE disposition_status_id IN (3,6) AND user_type = 'agent' AND enquiry_id=a.enquiry_id ORDER BY id ASC LIMIT 1)
           	
           	LEFT JOIN disposition_status_substatus_master as j ON j.id = (SELECT disposition_sub_status_id FROM lead_status WHERE disposition_sub_status_id IN (11,12,14,15,22,23,44,45) AND user_type = 'agent' AND enquiry_id=a.enquiry_id ORDER BY id ASC LIMIT 1)
           	
           	LEFT JOIN lead_status as k ON (k.enquiry_id = a.enquiry_id AND k.user_type='sales_person' AND NOT EXISTS ( SELECT 1 FROM lead_status p1 WHERE p1.enquiry_id = a.enquiry_id AND p1.id > k.id ))
           	
           	LEFT JOIN disposition_status_substatus_master as l ON (l.id = k.disposition_status_id)
           	
           	LEFT JOIN disposition_status_substatus_master as m ON (m.id = k.disposition_sub_status_id)
           	
           	LEFT JOIN campaign_master as n ON (n.id = a.leadPrimarySource)
           	
           	WHERE a.lead_assigned_to_asm_on >= '$startDate' AND a.lead_assigned_to_asm_on <='$endDate' GROUP BY a.enquiry_id ORDER BY a.enquiry_id ASC";
